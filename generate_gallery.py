...
FILE_EXTENSIONS = IMAGE_EXTENSIONS + ('.apk',)
...
def generate_gallery():
    ...
    for subdir in subdirs:
        ...
        files = [f for f in os.listdir(subdir_path) if f.lower().endswith(FILE_EXTENSIONS)]
        files.sort()

        # Check if this directory has subdirectories with images/files
        sub_subdirs = [d for d in os.listdir(subdir_path) if os.path.isdir(os.path.join(subdir_path, d)) and not d.startswith('.')]
        
        if files:
            # Directory has files directly
            categories.append({
                'dir': subdir,
                'title': subdir.capitalize(),
                'count': len(files)
            })
            generate_category_html(subdir, files)
            # 添加到JSON数据
            gallery_data[subdir] = [f"/{subdir}/{f}" for f in files]
        elif sub_subdirs:
            # Directory has subdirectories, check if they contain images/files
            total_files = 0
            has_files = False
            subdir_data = {}  # 存储子目录的图片/文件
            for sub_subdir in sub_subdirs:
                sub_subdir_path = os.path.join(subdir_path, sub_subdir)
                sub_files = [f for f in os.listdir(sub_subdir_path) if f.lower().endswith(FILE_EXTENSIONS)]
                if sub_files:
                    has_files = True
                    total_files += len(sub_files)
                    sub_files.sort()
                    # Generate index.html for each subdirectory
                    generate_category_html(os.path.join(subdir, sub_subdir), sub_files, parent_dir=subdir)
                    # 添加到JSON数据
                    subdir_data[sub_subdir] = [f"/{subdir}/{sub_subdir}/{f}" for f in sub_files]
...
"""
    ...
    for category in categories:
        icon = "📁"
        if category['dir'].lower() == 'apks':
            icon = "🤖"
        elif category['dir'].lower() in ['avatar', 'family', 'sights']:
            icon = "🖼️"
...
def generate_parent_category_html(parent_dir, subdirs):
    ...
    for subdir in subdirs:
        ...
        icon = "📁"
        if subdir.lower() == 'apks':
            icon = "🤖"
        elif subdir.lower() in ['avatar', 'family', 'sights']:
            icon = "🖼️"
...
def generate_category_html(subdir, files, parent_dir=None):
    ...
    for f in files:
        caption = get_title(f)
        file_url = urllib.parse.quote(f)
        is_image = f.lower().endswith(IMAGE_EXTENSIONS)
        
        if is_image:
            preview_html = f'<img src="{file_url}" alt="{caption}" loading="lazy">'
            copy_img_btn = f'<button class="action-btn copy-img" data-src="{file_url}">📋 Copy Image</button>'
            card_class = "photo-card"
        else:
            icon = "🤖" if f.lower().endswith('.apk') else "📄"
            preview_html = f'<div class="file-preview-icon">{icon}</div>'
            copy_img_btn = '' # No copy image for non-images
            card_class = "photo-card file-card"

        file_cards += f"""
        <div class="{card_class}" data-is-image="{"true" if is_image else "false"}">
            {preview_html}
            <div class="caption">{caption}</div>
            <div class="card-actions">
                {copy_img_btn}
                <button class="action-btn copy-link" data-src="{file_url}">🔗 Copy Link</button>
            </div>
        </div>"""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{title} - Resources Gallery</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    ...
    <script>
        ...
        cards.forEach(card => {{
            card.addEventListener('click', (e) => {{
                if (card.dataset.isImage !== "true") return;
